...
func EncodeAlawToBuffer(lpcm []byte, alaw []byte) {
	length := len(lpcm) / 2
	_ = alaw[length-1]

	for i := 0; i < length; i++ {
		alaw[i] = EncodeAlawFrame(int16(lpcm[i*2]) | int16(lpcm[i*2+1])<<8)
	}
}
...
func DecodeAlawToBuffer(alaw []byte, pcm []byte) {
	length := len(alaw)
	_ = pcm[length*2-1]

	for i := 0; i < length; i++ {
		frame := alaw2lpcm[alaw[i]]
		pcm[i*2] = byte(frame)
		pcm[i*2+1] = byte(frame >> 8)
	}
}
